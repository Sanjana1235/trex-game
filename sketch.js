...
function preload(){
  ...
  spaceshipImage = loadImage("mainspaceship.png")
}
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    spawnObstacles();
  spawnSpaceShips();
    if(obstaclesGroup.isTouching(trex)||trex.isTouching(spaceShipGroup)){
        gameState = END;
    }
  }
...
function spawnSpaceShips(){
  if(frameCount % 100==0&&frameCount>200){
     spaceShip = createSprite(600,100,10,10);
    spaceShip.addImage(spaceshipImage);
    spaceShip.velocityX = -4;
    spaceShip.scale = 0.05;
    spaceShip.lifetime = 200;
    spaceShip.debug = true
    spaceShip.setCollider("rectangle",0,0,80,200);
